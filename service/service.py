...
from db.db import DBMessage, MessageType
...
def create_ask_response(ai_response: str, user_token: str) -> ResponseMessage or ResponseDebugMessage:
    ...
    if env_value == "openai":
        ...
        response_messages = [DebugMessage(content=message.content, type=message.type) for message in conversation]                
...
class Service:
    ...
    def respond(self, message_content: str, user_token: str) -> ResponseMessage or ResponseDebugMessage: 
        user = UserModel.get(user_token)
        conversation = MessagesModel.get(user)
        
        ai_response = self.ai.respond(message_content, conversation)

        MessagesModel.upsert(user, [DBMessage(type=MessageType.client, content=message_content), DBMessage(type=MessageType.ai, content=ai_response)])
